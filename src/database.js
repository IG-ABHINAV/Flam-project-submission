...
export const dbPath = () => process.env.QUEUECTL_DB || '.queuectl/queuectl.sqlite';
...
export function openStore(file = dbPath()) {
  ...
  `);
  for (const [key, value] of Object.entries({ 'max-retries': '3', 'backoff-base': '2', shutdown: '0' })) {
...
export const settings = (db) => Object.fromEntries(db.prepare('SELECT key,value FROM settings ORDER BY key').all().map(x => [x.key, x.value]));

export function enqueue(db, job) {
  const time = isoNow();
  db.prepare(`INSERT INTO jobs (id,command,state,attempts,max_retries,priority,run_at,timeout_seconds,created_at,updated_at)
    VALUES (@id,@command,'pending',0,@max_retries,@priority,@run_at,@timeout_seconds,@created_at,@updated_at)`).run({ ...job, created_at: time, updated_at: time });
}
...
export function claimJob(db, workerId) {
  ...
  const claim = db.transaction(() => {
    const job = db.prepare(`SELECT * FROM jobs WHERE state IN ('pending','failed') AND run_at <= ?
      ORDER BY priority DESC, run_at, created_at LIMIT 1`).get(isoNow());
    if (!job) return null;
    const time = isoNow();
    const result = db.prepare(`UPDATE jobs SET state='processing', attempts=attempts+1, worker_id=?, started_at=?, updated_at=?
      WHERE id=? AND state IN ('pending','failed')`).run(workerId, time, time, job.id);
    return result.changes ? db.prepare('SELECT * FROM jobs WHERE id=?').get(job.id) : null;
  });
...
export function finishJob(db, job, workerId, result) {
  ...
  const runAt = succeeded || dead ? time : new Date(Date.now() + delayMs).toISOString();
  db.prepare(`UPDATE jobs SET state=?,run_at=?,completed_at=?,updated_at=?,exit_code=?,last_error=?,output=?,worker_id=? WHERE id=?`).run(
    state, runAt, succeeded || dead ? time : null, time, result.exitCode, result.error, result.output, workerId, job.id
  );
  db.prepare(`INSERT INTO attempt_logs(job_id,attempt,worker_id,started_at,finished_at,exit_code,state,output,error)
    VALUES (?,?,?,?,?,?,?,?,?)`).run(job.id, job.attempts, workerId, job.started_at || time, time, result.exitCode, state, result.output, result.error);
  return state;
}
